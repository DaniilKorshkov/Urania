...
def read_spectrum_json(filename):   #function to read array of spectrums from JSON files
    ...
    for line in handle:
        ...
        match tempdict["class"]:
            case "metadata":
                metadata = tempdict["valve_number"]
            case "spectrum":
                time = tempdict["time"]
...
def read_GUI_page_settings(filename, self_name):   #function to read page settings for GUI from JSON format
    ...
    for line in handle:
        ...
        try:
            tempdict = json.loads(line)
            if tempdict["page_name"] == self_name:
